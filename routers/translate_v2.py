...
async def translate_v2_stream(db: db_dependency, request: TranslateV2Request, current_user: User = Depends(get_current_active_user)):
    ...
    async def generate_stream() -> AsyncGenerator[str, None]:
        try:
            ...
            # Step 2: Fetch templates from database
            step_data = StreamStep(
                step="template_fetch",
                data={"message": "Fetching template details..."},
                status="progress"
            )
            yield f"data: {json.dumps(step_data.dict())}\n\n"
            await asyncio.sleep(0.1)
            ...
            step_data = StreamStep(
                step="translation_generation",
                data={"message": "Generating translations..."},
                status="progress"
            )
            yield f"data: {json.dumps(step_data.dict())}\n\n"
            await asyncio.sleep(0.1)
            ...
            step_data = StreamStep(
                step="translation_generation",
                data={
                    "message": "Both translations completed",
                    "translation_1_ready": translation_1 is not None,
                    "translation_2_ready": translation_2 is not None
                },
                status="completed"
            )
            yield f"data: {json.dumps(step_data.dict())}\n\n"
            ...
            # Step 5: Save battle result
            step_data = StreamStep(
                step="battle_result_save",
                data={"message": "Saving battle result..."},
                status="progress"
            )
            yield f"data: {json.dumps(step_data.dict())}\n\n"
            await asyncio.sleep(0.1)
...
async def generate_translation_async_stream(db: db_dependency, model: str, template: TemplateV2, input_text: str, translation_id: str) -> AsyncGenerator[StreamStep, None]:
    ...
    try:
        ...
        yield StreamStep(
            step=f"translation_{translation_id}_analysis",
            data={
                "ucca_required": is_ucca_present,
                "gloss_required": is_gloss_present,
                "commentaries_required": is_commentaries_present,
                "sanskrit_required": is_sanskrit_present
            },
            status="completed"
        )
        ...
        # Step 2: Get commentaries and sanskrit
        yield StreamStep(
        ...
        if is_ucca_present:
            yield StreamStep(
                ...
                data={"message": f"Generating UCCA for {model}..."},
                status="progress"
            )
            
            ucca_result = await get_ucca_async(input_text, commentaries_and_sanskrit, model)
            ucca = ucca_result.get("ucca_graph", None)
            
            yield StreamStep(
                step=f"translation_{translation_id}_ucca",
                data={"ucca_generated": ucca is not None},
                status="completed"
            )
        ...
        if is_gloss_present:
            yield StreamStep(
                ...
                data={"message": f"Generating Gloss for {model}..."},
                status="progress"
            )
            
            if is_ucca_present and ucca is not None:
                gloss_result = await get_gloss_async(input_text, commentaries_and_sanskrit, ucca, model)
            else:
                gloss_result = await get_gloss_async(input_text, commentaries_and_sanskrit, {}, model)
            
            gloss = gloss_result.get("glossary", None)
            
            yield StreamStep(
                step=f"translation_{translation_id}_gloss",
                data={"gloss_generated": gloss is not None},
                status="completed"
            )
        ...
        yield StreamStep(
            step=f"translation_{translation_id}_payload",
            data={"payload_prepared": True, "combo_key": combo_key},
            status="completed"
        )

        # Step 6: Generate translation
        yield StreamStep(
            step=f"translation_{translation_id}_generation",
            data={"message": f"Generating translation with {model}..."},
            status="progress"
        )
        ...
        yield StreamStep(
            step=f"translation_{translation_id}_error",
            data={"error": str(e)},
            status="error"
        )
